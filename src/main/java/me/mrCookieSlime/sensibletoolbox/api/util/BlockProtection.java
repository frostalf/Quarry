...
public class BlockProtection {
    ...
    public void setBlockProtectionType(BlockProtectionType blockProtectionType) {
        switch (blockProtectionType) {
            case BEST:
            		if (SensibleToolbox.getPluginInstance().isTownyAvailable()) 
            		if (SensibleToolbox.getPluginInstance().isWorldGuardAvailable()) {
            		this.blockProtectionType = BlockProtectionType.TOWNYWG;
            		} else {
	...
	public boolean playerCanBuild(Player player, Block block, Operation op) {
        switch (blockProtectionType) {
           case TOWNYWG:
        	   switch (op) {
        	   case PLACE:
           	    return WGBukkit.getPlugin().canBuild(player, block) && com.palmergames.bukkit.towny.utils.PlayerCacheUtil.getCachePermission(player, block.getLocation(), block.getTypeId() , block.getData(), TownyPermission.ActionType.BUILD );
               case BREAK:
            	    return WGBukkit.getPlugin().canBuild(player, block) && com.palmergames.bukkit.towny.utils.PlayerCacheUtil.getCachePermission(player, block.getLocation(), block.getTypeId() , block.getData(), TownyPermission.ActionType.DESTROY ); 
        	   }
            case BEST:
                throw new IllegalArgumentException("should never get here!");
            default:
    ...
    public enum BlockProtectionType {
        ...
        TOWNYWG,
        /**
         * Use Towny to check for protected regions.  If Towny is
         * not installed, BUKKIT protection is used as a fallback.
         */
        NONE;
        ...
        public boolean isAvailable() {
            switch (this) {
                case BEST: return SensibleToolbox.getPluginInstance().isTownyAvailable();
                case TOWNYWG: return SensibleToolbox.getPluginInstance().isTownyAvailable();
                default: return true;
            }
        }
    }
